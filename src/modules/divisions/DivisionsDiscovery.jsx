...
import { getLesson, TRICKS } from './divisionsData.js'

export default function DivisionsDiscovery() {
  ...
  useEffect(() => {
    if (step === totalSteps - 1) markDiscoverySeen('divisions', lessonId)
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8">
      <div className="max-w-2xl mx-auto">
        <BackButton to="/divisions" label="Leçons" />
        ...
        <div className="text-center mb-4">
          ...
          <div className="flex justify-center gap-1 mt-3">
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={`step-${i}`} className={`h-2 rounded-full ${i === step ? 'w-8 bg-purple-500' : 'w-2 bg-purple-200'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-between">
          ...
          {step < totalSteps - 1 ? (
            ...
            <Link to={`/divisions/entrainement/${lessonId}/1`} className="btn-kid bg-gradient-to-r from-purple-500 to-indigo-500 text-white">
              💪 S'entraîner !
            </Link>
          )}
        </div>
